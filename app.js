...
async function toggleTaskComplete(taskId) {
  // Convert taskId to number since it comes as string from dataset
  const numericTaskId = parseInt(taskId);
  const task = appData.tasks.find(t => t.id === numericTaskId);
  if (task) {
...
async function deleteTask(taskId) {
  if (confirm('Удалить задачу?')) {
    // Convert taskId to number since it comes as string from dataset
    const numericTaskId = parseInt(taskId);
    await deleteTaskFromSupabase(numericTaskId);
    appData.tasks = appData.tasks.filter(t => t.id !== numericTaskId);
    renderTasks();
    renderCompletedTasks();
  }
}
